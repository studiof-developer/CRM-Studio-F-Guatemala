...
//
// Recompresses every already-stored image attachment in place, the same way new ones
...
async function main() {
  ...
  console.log(`${rows.length} image attachment(s) to process`);
  let before = 0, after = 0, done = 0;
  for (const row of rows) {
    before += row.size_bytes ?? 0;
    await compressStoredImageAttachment(row.id);
