-- 2026-09-16/17 report (video + screenshot): a customer went Tibio → Caliente after
-- the advisor only asked "cómo quiere pagar" (no payment info given), and separately
-- after "Avísame para preparar el enlace de pago seguro... o si prefieres transferencia"
-- — announcing intent to send a link LATER, not an actual link. Same root cause both
-- times: the regex fired on the advisor merely asking about payment or promising a
-- link, not on the advisor actually handing over payment info. Two patterns dropped:
--   - `cómo (vas a|deseas|quieres|prefieres) pagar` — a question, not information.
--   - `(link|enlace) (de|para) (el )?pago` — matched the bare phrase regardless of
--     tense ("preparo el enlace de pago" reads the same as "aquí tu enlace de pago" to
--     a regex); the existing `https?://` pattern already catches a REAL link being
--     shared, which is what should actually mean "caliente".
-- Same trigger as db/init/044, only this one line changed — no other behavior touched.
-- (Kept in sync with db/init/057, which exists for a brand-new install; this file is
-- what actually applies it to an already-running deployment — see migrate.js.)
...
BEGIN
  ...
  IF v_is_real_advisor THEN
    UPDATE tickets SET
      ...
      updated_at = now()
    WHERE customer_id = v_customer.id AND status != 'resuelto' AND assigned_advisor IS NULL
  ...
  IF v_rank < 2 AND v_content ~* '(m[eé]todo\s+de\s+pago|medio\s+de\s+pago|puedes?\s+pagar\s+(por|con)\s+(transferencia|dep[oó]sito|efectivo|tarjeta)|https?://|n[uú]mero\s+de\s+cuenta|completar\s+tu\s+env[ií]o|nit\s+o\s+dpi)' THEN
