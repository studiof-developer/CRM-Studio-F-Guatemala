...
-- 049 only backfilled assigned_advisor while claiming a ticket OUT of esperando_asesor/
-- difusion_enviada — a ticket that was ALREADY en_atencion with no name (from the 047
-- bug, or any other future path that could leave one unassigned) never got touched
-- again, even once a real advisor started actually replying to it. Real report
-- (2026-09-11, "los que dicen 'Asesor', asignarlos al que está respondiendo" — 116
-- backfilled by hand). Rather than chase every possible path that could leave
-- assigned_advisor null, this makes it self-healing: ANY genuine advisor's message
-- fills in the name if it's still missing, regardless of the ticket's current status
-- (as long as it isn't resuelto) — so whatever the cause, the very next real reply
...
BEGIN
  ...
  -- Only a genuine registered staff member's own reply claims/fills in a ticket — a
  -- bot/automation tagging itself sentBy:'advisor' (any name or none) never touches
  -- this. Broadened past "only esperando_asesor/difusion_enviada" to any still-open
  -- ticket missing a name: self-heals regardless of how it ended up unassigned.
