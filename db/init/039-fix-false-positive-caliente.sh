...
-- Fixes a false positive from 038's own broadening, reported same day: "También puedes
-- pagar CON EL DESCUENTO y..." (a promotion, nothing to do with payment method) matched
-- the new "puedes pagar por/con X" pattern because X was left unconstrained — any noun
-- after "pagar con/por" counted. Now requires X to actually be a payment method
-- (transferencia/depósito/efectivo/tarjeta), same words the rest of this trigger and
-- the OCR method-guesser already treat as real payment vocabulary.
